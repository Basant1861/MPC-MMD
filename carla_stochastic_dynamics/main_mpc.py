...
def main():
    ...
    for num_reduced in list_num_reduced:
        ...
        if cost=="mmd_opt":
            ...
            func_cem = prob.compute_cem_cvar_opt
        ...
        for ii in range(num_exps):
            ...
            if town == "Town10HD":
                ...
                goal_position = carla.Location(x=106, y=25)
        ...
        #########################################################3333
            ...
            while( dist_goal >= 7. and i<=5000 ):
                ...
                behavioural_inputs, _beta_cem_data,res_cem\
                ...
                a_ego = np.append(a_ego,a_control*np.ones(1).reshape(1,-1),axis=0)
            ...
            # obstacle_init_frenet_all.append(obstacle_init_frenet)
            # ego_init_frenet_all.append(ego_init_frenet)
            # curvature_ref_all.append(curvature_ref)
            # path_ref_all.append(path_ref)
            # traj_input_all.append(traj_input)
            # neural_output_all.append(neural_output)
            # beta_cem_data_all.append(beta_cem_data)

            # count = len(obstacle_init_frenet_all)
            # arr_obstacle_init_frenet_all = np.empty(count,object)
            # arr_ego_init_frenet_all = np.empty(count,object)
            # arr_curvature_ref_all = np.empty(count,object)
            # arr_path_ref_all = np.empty(count,object)
            # arr_traj_input_all = np.empty(count,object)
            # arr_neural_output_all = np.empty(count,object)
            # arr_beta_cem_data_all = np.empty(count,object)

            # arr_obstacle_init_frenet_all[:] = obstacle_init_frenet_all
            # arr_ego_init_frenet_all[:] = ego_init_frenet_all
            # arr_curvature_ref_all[:] = curvature_ref_all
            # arr_path_ref_all[:] = path_ref_all
            # arr_traj_input_all[:] = traj_input_all
            # arr_neural_output_all[:] = neural_output_all
            # arr_beta_cem_data_all[:] = beta_cem_data_all

            # np.savez("./data/neural_network/nn_static_2",obstacle_init_frenet_all = arr_obstacle_init_frenet_all,
            #          ego_init_frenet_all=arr_ego_init_frenet_all,curvature_ref_all=arr_curvature_ref_all,
            #          path_ref_all=arr_path_ref_all,traj_input_all=arr_traj_input_all,neural_output_all=arr_neural_output_all
            #          ,dtype=object)
            
            # np.savez("./data/neural_network/{}_mother_{}_reduced_{}_horizon_{}".format(noise,prob.num_reduced,
            #                                                                                    prob.num_reduced_sqrt,prob.num_prime),
            #          beta_cem_data = arr_beta_cem_data_all,dtype=object)
                        
            np.savez("./data/{}/{}_noise/noise_{}/ts_{}/{}_{}_{}_samples_{}".format(town,noise,int(noise_level*100),
                                                                            prob.num_prime,obs_type,
                                                                            cost,num_reduced,ii),
                                                        x_ego=x_ego,y_ego=y_ego,psi_ego=psi_ego,
                                                        v_ego=v_ego,psidot_ego=psidot_ego,steer_ego=steer_ego,a_ego = a_ego,
            ...
            # np.savez("./stats/{}/{}_noise/noise_{}/ts_{}/{}_{}_{}_samples".format(town,noise,int(noise_level*100),
            #                                                                     prob.num_prime,obs_type,
            #                                                                     cost,num_reduced),
            #                             lane_violations=sum(list_lane_violations)/len(list_lane_violations),
            #                             num_collisions=num_collisions/len(list_lane_violations),
            #                             avg_vel = sum(list_speed)/len(list_speed))
